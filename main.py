...
input_neurons_size = 100
plot_array_shape = (10, 10)
...
def experiment(stimulus_1, stimulus_2, weights):
    ...
    second_stage_learning_stimulus_2 = np.dot(stimulus_2, weights_zeros_for_random_neurons)
...
def plot_heatmap():
    ...
    sns.heatmap(heatmap_2, ax=ax, cmap=cmap, annot=True, fmt="d", linewidth=0.5, cbar_kws={'label': 'Activations Number'})
    fig.tight_layout()
    sns.heatmap(heatmap_1, ax=ax2, cmap=cmap, annot=True, fmt="d", linewidth=0.5, cbar_kws={'label': 'Activations Number'})
    fig.tight_layout()
